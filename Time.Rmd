...
###T test-- are variable means equal for weekdays and weekends?

```{r}
test = function(variable){
  test_df = ready2_df %>% 
    filter(info2 == variable)
  daytest = test_df %>% 
    filter(days == 'Weekday') %>% 
    select(value2)
  ...
  result = t.test(daytest$value2, endtest$value2)
  
  result}
  
renderPrint(
  if(test(variable = input[["info2"]]))

renderPrint(if(test(variable = input[["info2"]])$p.value<0.05){print('reject null')}else{print('fail to reject null')})
)
