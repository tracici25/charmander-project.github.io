...
r_rh_m = 
  ...
  mutate(month = factor(month, 
                        levels = c('Jan', 'Feb', 'Mar', 'Apr',
                                   ...
                                   'Sep', 'Oct', 'Nov', 'Dec'),
                        ordered = TRUE)) %>% 
  select(rain, month, RH) %>% 
  group_by(month) %>% 
  summarise(rain = mean(rain), rh=mean(RH))


fig <- plot_ly(r_rh_m)
fig <- fig %>% add_trace(x = ~month, y = ~rain, type = 'bar', name = 'Rain',
            marker = list(color = '#C9EFF9'),
            hoverinfo = "text",
            text = ~paste(rain, 'mm/m2'))
fig <- fig %>% add_trace(x = ~month, y = ~rh, type = 'scatter',name = 'Relative Humidity', mode = 'lines', yaxis = 'y2',
            line = list(color = '#45171D'),
            hoverinfo = "text",
            text = ~paste('Relative Humidity: ', rh))
fig <- fig %>% layout(title = 'Rain & Relative Humidity vs. Month',
         xaxis = list(title = ""),
         yaxis = list(side = 'left', title = 'Rain', showgrid = FALSE, zeroline = FALSE),
         yaxis2 = list(side = 'right', overlaying = "y", title = 'Relative Humidity', showgrid = FALSE, zeroline = FALSE))
