...
rain_month_plot =
  ggplot(rm, aes(x = month, y = rain)) + 
  geom_point(aes(color = month))+ labs(title = 'Rain vs Month', x = 'Month', y = 'Rain(mm/m2)') + geom_line()
ggplotly(rain_month_plot)

rh_month_plot = 
  ggplot(rhm, aes(x = month, y = rh)) + 
  geom_point(aes(color = month))+ labs(title = 'RH vs Month', x = 'Month', y = 'RH(%)')
ggplotly(rh_month_plot)
