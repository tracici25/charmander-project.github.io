  ...
  plot_ly(
    ...
    type = 'scatter', colors = 'viridis', color = ~days, mode = 'markers', hoverinfo = 'text',
        text = ~paste('</br> Month: ', month,
                      ...
                      '</br> Rain (mm/m2): ', rain))
```
calculate the days of fire (weekday and weekend) and compare their ratio to the natural one.
...
ggplot(fire_mon, aes(x = month, fill = month)) + 
  geom_bar() + scale_fill_viridis_d(direction = -1) +
  xlab('Month') + ylab('Count (s)')
```

Add text label on bar chart.
change month order.
